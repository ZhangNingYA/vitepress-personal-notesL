...
## 1. 按虚拟化对象分类 (按资源划分)
* **服务器虚拟化 (计算虚拟化)**：在一台物理服务器上运行多个虚拟机 (VM)，是云计算的核心。
    * *代表技术：KVM, VMware vSphere.*
* **存储虚拟化**：将多个物理存储设备整合为一个逻辑存储池。
    * *代表技术：SAN, Ceph (分布式存储).*
* **网络虚拟化**：在物理网络上构建逻辑网络 (SDN)。
    * *代表技术：VLAN, VMware NSX.*
* **应用/桌面虚拟化**：
    * **应用虚拟化**：如 Docker 容器。
    * **桌面虚拟化 (VDI)**：远程访问云端电脑桌面。

---

## 2. 按 Hypervisor 架构分类 (按安装位置划分)
| 分类 | 名称 | 安装位置 | 特点 | 代表产品 |
| :--- | :--- | :--- | :--- | :--- |
| **I 型** | 裸金属型 (Bare-metal) | 直接安装在物理硬件上 | 效率极高，稳定性强，企业级首选 | ESXi, Hyper-V, Xen |
| **II 型** | 宿主型 (Hosted) | 安装在操作系统之上 (如 Windows) | 安装方便，性能有损耗，适合个人测试 | VMware Workstation, VirtualBox |
...
* **原理**：虚拟机不知道自己是虚拟的，Hypervisor 模拟完整硬件。
* **优点**：兼容性极好，无需修改客户机内核。
* **缺点**：软件模拟拦截指令，开销较大。

### **B. 半虚拟化 (Para-virtualization)**
* **原理**：虚拟机知道自己是虚拟的，通过 **Hypercall (超级调用)** 与 Hypervisor 协作。
* **优点**：性能极高，省去模拟开销。
* **缺点**：必须修改客户机内核，Windows 等闭源系统难以支持。

## 4. 补充：操作系统级虚拟化 (容器化)
* **代表**：Docker, Kubernetes (K8s)。
* **特点**：不模拟硬件，共享宿主机内核。极其轻量（MB 级），秒级启动。
